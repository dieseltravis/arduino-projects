...
void loop()
{
  ...
  //set colors
        accel_t_gyro.value.x_accel = constrain (accel_t_gyro.value.x_accel, 261, 399);  // keep the numbers within a range so it doesn't take huge
        accel_t_gyro.value.y_accel = constrain (accel_t_gyro.value.y_accel, 261, 399);  // jolts to get bright colors
        accel_t_gyro.value.z_accel = constrain (accel_t_gyro.value.z_accel, 264, 400);  // the values returned for resting rotation through the range of motion are
                                                  // X [266-400] Y [263-395] z [264-395]  so 395 is 1g
        accel_t_gyro.value.x_accel = map (accel_t_gyro.value.x_accel, 260, 400, 100, 240);  // analogWrite wants values from 0 - 255
        accel_t_gyro.value.y_accel = map (accel_t_gyro.value.y_accel, 260, 400, 100, 255);  // so this keeps the values returned by analogRead (sensorXYZ)
        accel_t_gyro.value.z_accel = map (accel_t_gyro.value.z_accel, 263, 401, 100, 255);  // from rolling over and making weird flashes
        Serial.print(accel_t_gyro.value.x_accel, DEC );  // this is unnecessary to the operation of the code
        Serial.print("\t");           // but it helps to figure out which colors are responding.
        Serial.print(accel_t_gyro.value.y_accel, DEC  );
        Serial.print("\t");
        Serial.println(accel_t_gyro.value.z_accel, DEC  );
        
        analogWrite (redPin, accel_t_gyro.value.x_accel);
        analogWrite (greenPin, accel_t_gyro.value.y_accel);
        analogWrite (bluePin, accel_t_gyro.value.z_accel);
